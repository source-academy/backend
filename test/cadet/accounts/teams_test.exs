defmodule Cadet.Accounts.TeamTest do
    ...
    test "creating a new team with duplicate students in the one row", %{user1: user1, assessment1: assessment1, course1: course1} do
        {:ok, course_reg1} =
            CourseRegistrations.insert_or_update_course_registration(%{
                user_id: user1.id,
                course_id: course1.id,
        ...
        assert result == {:error, {:conflict, "One or more students appear multiple times in a team!"}}
    end

    test "creating a new team with duplicate students across the teams but not in one row", %{user1: user1, user2: user2, user3: user3, assessment1: assessment1, course1: course1} do
        {:ok, course_reg1} =
            CourseRegistrations.insert_or_update_course_registration(%{
                user_id: user1.id,
                course_id: course1.id,
    ...
    test "creating a team with students already in another team for the same assessment", %{user1: user1, user2: user2, user3: user3, assessment1: assessment1, course1: course1}  do
        ...
        assert result == {:error, {:conflict, "One or more students already in a team for this assessment!"}}

    end

    
    test "creating a team with students exceeding the maximum team size", %{user1: user1, user2: user2, user3: user3, assessment2: assessment2, course1: course1}  do
        ...
        "student_ids" => [
            [%{"userId" => course_reg1.id},%{"userId" => course_reg2.id},%{"userId" => course_reg3.id}],
        ]
        }
    ...
    test "inserting a team with non-exisiting student", %{user1: user1, user2: user2, assessment1: assessment1, course1: course1} do
    ...
    test "inserting a team with an exisiting student but not enrolled in this course", %{user1: user1, user2: user2, assessment1: assessment1, course1: course1, course2: course2} do
        
        {:ok, course_reg1} = CourseRegistrations.insert_or_update_course_registration(%{user_id: user1.id, course_id: course1.id, role: :student})
        {:ok, course_reg2} = CourseRegistrations.insert_or_update_course_registration(%{user_id: user2.id, course_id: course2.id, role: :student})
    ...
    test "update an existing team with valid new team members", %{user1: user1, user2: user2, user3: user3, assessment1: assessment1, course1: course1} do
        {:ok, course_reg1} = CourseRegistrations.insert_or_update_course_registration(%{user_id: user1.id, course_id: course1.id, role: :student})
        {:ok, course_reg2} = CourseRegistrations.insert_or_update_course_registration(%{user_id: user2.id, course_id: course1.id, role: :student})
        {:ok, course_reg3} = CourseRegistrations.insert_or_update_course_registration(%{user_id: user3.id, course_id: course1.id, role: :student})
    ...
    test "update an existing team with new team members who are already in another team", %{user1: user1, user2: user2, user3: user3, assessment1: assessment1, course1: course1} do
        {:ok, course_reg1} = CourseRegistrations.insert_or_update_course_registration(%{user_id: user1.id, course_id: course1.id, role: :student})
        {:ok, course_reg2} = CourseRegistrations.insert_or_update_course_registration(%{user_id: user2.id, course_id: course1.id, role: :student})
        {:ok, course_reg3} = CourseRegistrations.insert_or_update_course_registration(%{user_id: user3.id, course_id: course1.id, role: :student})
        ...
        assert result == {:error, {:conflict, "One or more students are already in another team for the same assessment!"}}
    end

    test "delete an existing team", %{user1: user1, user2: user2, user3: user3, assessment1: assessment1, course1: course1} do
        {:ok, course_reg1} = CourseRegistrations.insert_or_update_course_registration(%{user_id: user1.id, course_id: course1.id, role: :student})
        {:ok, course_reg2} = CourseRegistrations.insert_or_update_course_registration(%{user_id: user2.id, course_id: course1.id, role: :student})
        {:ok, course_reg3} = CourseRegistrations.insert_or_update_course_registration(%{user_id: user3.id, course_id: course1.id, role: :student})
