defmodule CadetWeb.StoriesControllerTest do
  ...
  describe "GET /stories" do
    ...
    test "student permission, only obtain published stories", %{conn: conn, valid_params: params} do
      insert(:story)
      insert(:story)
  ...
  describe "POST /stories/new" do
    ...
    test "student permission, forbidden", %{conn: conn, valid_params: params} do
      conn = delete(conn, build_url("new"), params)
      assert response(conn, 403) =~ "User not allowed to manage stories"
    end
    ...
    test "creates a new story", %{conn: conn, valid_params: params} do
      {:ok, resp} =
        conn
        |> post(build_url("new"), params)
        |> response(200)
        |> Jason.decode()

      # assert Map.delete(resp, "id") == render(conn, "show.json", %{story: params})
    end
  end
  ...
  describe "POST /stories/:storyid" do
    @tag authenticate: :student
    test "student permission, forbidden", %{conn: conn, valid_params: params} do
      conn = delete(conn, build_url(1), params)
      assert response(conn, 403) =~ "User not allowed to manage stories"
    end
    ...
    #   # assert response(conn, 200) =~ "User not allowed to manage stories"
    # end
  end
