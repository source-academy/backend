defmodule Cadet.Workers.NotificationWorker do
  ...
  defp is_course_enabled(notification_type_id, course_id, assessment_config_id) do
    Cadet.Notifications.get_notification_config!(notification_type_id, course_id, assessment_config_id).is_enabled
  end
  ...
  def is_user_time_option_matched(notification_type_id, assessment_config_id, course_reg_id, time_option_minutes) do
    pref = Cadet.Notifications.get_notification_preference(notification_type_id, course_reg_id)

    cond do
      (is_nil(pref)) ->
        Cadet.Notifications.get_default_time_option_for_assessment!(assessment_config_id, notification_type_id).minutes == time_option_minutes
      true ->
  ...
  def perform(%Oban.Job{
    ...
    for course_id <- Cadet.Courses.get_all_course_ids() do
      ...
      for avenger_cr <- avengers_crs do
        ...
        ungraded_submissions = Jason.decode!(elem(Cadet.Assessments.all_submissions_by_grader_for_index(avenger_cr, true, true), 1))
