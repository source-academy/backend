defmodule Cadet.Efficiency do
  ...
  def update_runtimes(assessmentId, questionId, userId) when is_ecto_id(assessmentId) and is_ecto_id(questionId) and is_ecto_id(userId) do
    #Repo.insert!(
    ...
    schema = Repo.get_by(Runtimes, assessment_id: assessment_id, question_id: question_id, user_id: user_id)
  ...
  defp efficiency_real_data(id, question_id) when is_ecto_id(id) and is_ecto_id(question_id) do
    ...
    |> select([r, s, a], {r.user_id, r.counter, s.inserted_at, s.updated_at, a.xp, a.xp_adjustment})
    |> Repo.all()
    |> Enum.map(fn({a, b, c, d, e, f}) -> %{sid: a, runtimes: b, inserted_at: c, updated_at: d, xp: e, xp_adjustment: f} end)
    |> Jason.encode!()
  end
