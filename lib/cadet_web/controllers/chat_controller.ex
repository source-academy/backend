defmodule CadetWeb.ChatController do
  ...
  swagger_path :chat do
    ...
    parameters do
      message(:body, :list, "Conversation history. Need to be an non empty list of format {role: string, content:string}. For more details, refer to https://platform.openai.com/docs/api-reference/chat/create")
    end
    ...
    case m do
      ...
      _ ->
        case is_message_list?(m) do
          ...
          false ->
            send_resp(conn, :bad_request, "Request must be a non empty list of message{role:string, content:string}")
        end
    end
  end
  defp is_message_list?(list) do
    is_list(list) &&
    Enum.all?(list, fn %{ "content" => _content, "role" => _role } -> true; _ -> false end) &&
    length(list) > 0
  end

  defp convert(list) do
    Enum.map(list, fn %{ "content" => content, "role" => role } ->
      %{ role: role, content: content } end)
  end
